...
if [ -z "$1" ]; then
  echo "usage: $(basename "$0") <function name> [dep name...]"
  exit 1
fi
...
copy_file "./functions/${NAME}.py" ./temp/build/

TEMP_FUNC_ZIP_NAME="${NAME}-function.zip"

build_zip ./temp/build/ "./dist/upload/$TEMP_FUNC_ZIP_NAME"

VERSION=$(get_version "./dist/upload/$TEMP_FUNC_ZIP_NAME")
echo :: determined version for function to be "$VERSION"

echo "$VERSION" >| "./temp/versions/function-${NAME}.version"
move_file "./dist/upload/$TEMP_FUNC_ZIP_NAME" "./dist/upload/${NAME}-function_${VERSION}.zip"

echo :: cleaning up temp directory
rm -rf ./temp/build || true

echo :: function build complete for "$NAME", took ~"$SECONDS" seconds
